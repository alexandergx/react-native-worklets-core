...
const globals = new Set(["this", "console", "performance", "_chronoNow", "BigInt", "Date", "Array", "ArrayBuffer", "Int8Array", "Int16Array", "Int32Array", "Uint8Array", "Uint8ClampedArray", "Uint16Array", "Uint32Array", "Float32Array", "Float64Array", "BigInt64Array", "BigUint64Array", "Date", "HermesInternal", "JSON", "Math", "Number", "Object", "String", "Symbol", "undefined", "null", "UIManager", "requestAnimationFrame", "_WORKLET", "arguments", "Boolean", "parseInt", "parseFloat", "Map", "WeakMap", "WeakRef", "Set", "_log", "_scheduleOnJS", "_makeShareableClone", "_updateDataSynchronously", "eval", "_updatePropsPaper", "_updatePropsFabric", "_removeShadowNodeFromRegistry", "RegExp", "Error", "ErrorUtils", "global", "_measure", "_scrollTo", "_dispatchCommand", "_setGestureState", "_getCurrentTime", "_eventTimestamp", "_frameTimestamp", "isNaN", "LayoutAnimationRepository", "_notifyAboutProgress", "_notifyAboutEnd", "Promise"]);

// leaving way to avoid deep capturing by adding 'stopCapturing' to the blacklist
const blacklistedFunctions = new Set(["stopCapturing", "toString", "map", "filter", "findIndex", "forEach", "valueOf", "toPrecision", "toExponential", "constructor", "toFixed", "toLocaleString", "toSource", "charAt", "charCodeAt", "concat", "indexOf", "lastIndexOf", "localeCompare", "length", "match", "replace", "search", "slice", "split", "substr", "substring", "toLocaleLowerCase", "toLocaleUpperCase", "toLowerCase", "toUpperCase", "every", "join", "pop", "push", "reduce", "reduceRight", "reverse", "shift", "slice", "some", "sort", "splice", "unshift", "hasOwnProperty", "isPrototypeOf", "propertyIsEnumerable", "bind", "apply", "call", "__callAsync", "includes"]);
class ClosureGenerator {
  ...
  findPrefixRec(path) {
    ...
    if (memberExpressionNode.computed || memberExpressionNode.property.name === "value" || blacklistedFunctions.has(memberExpressionNode.property.name)) {
  ...
  generateNodeForBase(t, current, parent) {
    ...
    return t.objectExpression(Object.keys(currentNode[0]).map(propertyName => t.objectProperty(t.identifier(propertyName), this.generateNodeForBase(t, propertyName, currentNode), false, true)));
  }
  generate(t, variables, names) {
    const arrayOfKeys = [...names];
    return t.objectExpression(variables.map((variable, index) => t.objectProperty(t.identifier(variable.name), this.generateNodeForBase(t, arrayOfKeys[index], this.trie), false, true)));
  }
}
function isRelease() {
...
function buildWorkletString(t, fun, closureVariables, name, inputMap) {
  function prependClosureVariablesIfNecessary() {
    const closureDeclaration = t.variableDeclaration("const", [t.variableDeclarator(t.objectPattern(closureVariables.map(variable => t.objectProperty(t.identifier(variable.name), t.identifier(variable.name), false, true))), t.memberExpression(t.thisExpression(), t.identifier("__closure")))]);
    ...
    function prepandRecursiveDeclaration(path) {
      if (path.parent.type === "Program" && path.node.id && path.scope.parent) {
        var _path$scope$parent$bi;
        const hasRecursiveCalls = ((_path$scope$parent$bi = path.scope.parent.bindings[path.node.id.name]) === null || _path$scope$parent$bi === void 0 ? void 0 : _path$scope$parent$bi.references) > 0;
        if (hasRecursiveCalls) {
          path.node.body.body.unshift(t.variableDeclaration("const", [t.variableDeclarator(t.identifier(path.node.id.name), t.memberExpression(t.thisExpression(), t.identifier("_recur")))]));
        }
      }
    }
    return {
  ...
  const expression = fun.program.body.find(({
    type
  }) => type === "FunctionDeclaration") || fun.program.body.find(({
    type
  }) => type === "ExpressionStatement").expression;
  const workletFunction = t.functionExpression(t.identifier(name), expression.params, expression.body);
...
function makeWorklet(t, fun, state) {
  ...
  const initDataObjectExpression = t.objectExpression([t.objectProperty(t.identifier("code"), t.stringLiteral(funString)), t.objectProperty(t.identifier("location"), t.stringLiteral(location))]);
  if (sourceMapString) {
    initDataObjectExpression.properties.push(t.objectProperty(t.identifier("sourceMap"), t.stringLiteral(sourceMapString)));
  }
  pathForStringDefinitions.insertBefore(t.variableDeclaration("const", [t.variableDeclarator(initDataId, initDataObjectExpression)]));
  const statements = [t.variableDeclaration("const", [t.variableDeclarator(privateFunctionId, funExpression)]), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("__closure"), false), closureGenerator.generate(t, variables, closure.keys()))), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("__initData"), false), initDataId)), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("__workletHash"), false), t.numericLiteral(workletHash)))];
  if (!isRelease()) {
    statements.unshift(t.variableDeclaration("const", [t.variableDeclarator(t.identifier("_e"), t.arrayExpression([t.newExpression(t.identifier("Error"), []), t.numericLiteral(lineOffset), t.numericLiteral(-20) // the placement of opening bracket after Exception in line that defined '_e' variable
    ]))]));
...
function processWorkletFunction(t, fun, state) {
  ...
  fun.replaceWith(fun.node.id && needDeclaration ? t.variableDeclaration("const", [t.variableDeclarator(fun.node.id, replacement)]) : replacement);
}
function processWorkletObjectMethod(t, path, state) {
  ...
  const replacement = t.objectProperty(t.identifier(path.node.key.name), t.callExpression(newFun, []));
  ...
  fun.traverse({
    DirectiveLiteral(path) {
      ...
      if (value === "worklet" && path.getFunctionParent() === fun) {
        ...
        if (directives && directives.length > 0 && directives.some(directive => t.isDirectiveLiteral(directive.value) && directive.value.value === "worklet")) {
...
function isGestureObjectEventCallbackMethod(t, node) {
  ...
  return t.isMemberExpression(node) && t.isIdentifier(node.property) && containsGestureObject(t, node.object);
}
...
function isGestureObject(t, node) {
  ...
  */
  return t.isCallExpression(node) && t.isMemberExpression(node.callee) && t.isIdentifier(node.callee.object) && node.callee.object.name === "Gesture" && t.isIdentifier(node.callee.property);
}
function processWorklets(t, path, state) {
  const callee = path.node.callee.type === "SequenceExpression" ? path.node.callee.expressions[path.node.callee.expressions.length - 1] : path.node.callee;
...
module.exports = function ({
  ...
  return {
    pre() {
      ...
      // Extra globals.
      (_this$opts = this.opts) === null || _this$opts === void 0 || (_this$opts = _this$opts.globals) === null || _this$opts === void 0 || _this$opts.forEach(name => {
        globals.add(name);
      });
      ...
      (_this$opts2 = this.opts) === null || _this$opts2 === void 0 || (_this$opts2 = _this$opts2.functionsToWorkletize) === null || _this$opts2 === void 0 || _this$opts2.forEach(({
        name,
        args
      }) => {
